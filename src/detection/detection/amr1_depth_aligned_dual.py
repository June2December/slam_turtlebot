...
# ── 설정 ──────────────────────────────────────────────────────────────────
NS             = "/robot4"
YOLO_WEIGHTS   = "./db/yolo11n.pt"
RESNET_WEIGHTS = "./db/yolo11n_resnet18.pt"
CONF_THRESH    = 0.5
DX, DY         = 0, 5
LOG_INTERVAL   = 0.5
WIN_W, WIN_H   = 960, 540

# 공통 클래스 라벨 (YOLO / ResNet18 동일, 인덱스 순서 맞춰주세요)
...
def apply_offset(depth, dx, dy):
    ...
    return cv2.warpAffine(depth, M, (w, h),
...
class DetectDepthNode(Node):
    def __init__(self):
        super().__init__("detect_depth_node")
    ...
    # ── GUI ───────────────────────────────────────────────────────────────
    def _gui(self):
        WIN = "Detection + Depth (YOLO + ResNet18)"
