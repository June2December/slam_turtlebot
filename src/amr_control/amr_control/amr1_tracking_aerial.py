...
SCAN_SPEED       = 0.5
MIN_SCAN_SPEED   = 0.1
SWEEP_RAMP_ANGLE = math.pi / 4   # 45° 가속/감속 구간
SWEEP_ANGLE      = 2 * math.pi   # 360°
LOST_TIMEOUT     = 3.0
...
class TargetTracker(Node):
    def __init__(self):
        ...
        self.cmd_interval = 0.05  # 100ms — 네트워크+로봇 반응 지연

        # 소실 처리
        self.last_detected_time = None
        self.final_sweep_start = None
        self.last_angular_z = SCAN_SPEED  # 기본값
        self.sweep_accumulated = 0.0
        self.sweep_prev_time   = None
    ...
    def imu_callback(self, msg):
        if self.final_sweep_start is None:
            self.sweep_prev_time = None
            return

        now = time.time()
        if self.sweep_prev_time is None:
            self.sweep_prev_time = now
            return

        dt = now - self.sweep_prev_time
        self.sweep_prev_time = now
        self.sweep_accumulated += abs(msg.angular_velocity.z * dt)
    ...
    def synced_callback(self, rgb_msg, depth_msg):
        ...
        # 즉시 회전 명령 (TF/depth 와 무관하게 첫 프레임부터 실행)
        ...
        valid = patch[patch > 0]
        if valid.size == 0:
            self.get_logger().warn(f'no valid depth at ({u},{v})')
            return
        depth_value_raw = float(np.median(valid))
        depth_value = self.apply_depth_maf(depth_value_raw)
        print(f"픽셀 중앙: ({u},{v}) depth raw={depth_value_raw:.0f}mm maf={depth_value:.0f}mm")
    ...
    def _handle_lost(self):
        ...
        if self.final_sweep_start is not None:
            acc = self.sweep_accumulated
            remaining = SWEEP_ANGLE - acc
            print(f'리트레킹 | {math.degrees(acc):.1f}° / 360°')
            if acc < SWEEP_ANGLE:
                # trapezoidal
                if acc < SWEEP_RAMP_ANGLE:
                    speed = max(MIN_SCAN_SPEED, (acc / SWEEP_RAMP_ANGLE) * SCAN_SPEED)
                elif remaining < SWEEP_RAMP_ANGLE:
                    speed = max(MIN_SCAN_SPEED, (remaining / SWEEP_RAMP_ANGLE) * SCAN_SPEED)
                else:
                    speed = SCAN_SPEED
        ...
        if now - self.last_detected_time >= LOST_TIMEOUT:
            ...
            self.publish_rotation(MIN_SCAN_SPEED)
            return
