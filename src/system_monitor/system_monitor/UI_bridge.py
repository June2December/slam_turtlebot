...
robots: Dict[str, RobotState] = {
    "amr1": RobotState(robot_id="amr1", name="AMR_1", battery=0),
    "amr2": RobotState(robot_id="amr2", name="AMR_2", battery=0),
}
...
class UiBridgeNode(Node):
    def __init__(self):
        ...
        # AMR 상태 / pose / battery
        # ----------------------------------------------------
        self.create_subscription(   # 상태 LOG for AMR 1
            String,
            "/amr1/state",
        ...
        self.create_subscription( # for AMR 2
            String,
            "/amr2/state",
        ...
        self.create_subscription(   # 배터리 잔량 AMR1
            Int32,
            "/robot4/battery_status",
            self.amr1_battery_callback,
            10
        )

        self.create_subscription(   # 배터리 잔량
            Int32,
            "/robot1/battery_status",
        ...
        # AMR_2 mission 관련
        # ----------------------------------------------------
        self.create_subscription(   # 1~7/7 waypoints status
            Int32,
            "/amr2/current_goal",
        ...
        self.create_subscription( # AMR1 -> main
            PointStamped,
            "/amr1/track_pose",     # tracking
            self.air_pose_callback,
            10
        )

        self.create_subscription(   # AMR2 -> main
            PointStamped,
            "/amr2/object_pose",    # target position(unknown)
    ...
    def amr1_battery_callback(self, msg: Int32):
        robots["amr1"].battery = max(0, min(100, int(msg.data * 100)))

    def amr2_battery_callback(self, msg: Int32):
        robots["amr2"].battery = max(0, min(100, int(msg.data * 100)))
...
class UiBridgeNode(Node):
    def __init__(self):
        ...
        self.battery_1 = self.create_subscription(
            Int32,
            "/amr1/battery_status",
            self.amr1_battery_callback,
            10
        ...
        self.battery_2 = self.create_subscription(
            Int32,
            "/amr2/battery_status",
            self.amr2_battery_callback,
            10
